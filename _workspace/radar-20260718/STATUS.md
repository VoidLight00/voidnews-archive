...
- fixture selftest: 완전 기록 기대 exit 0 / 1건 누락 기대 exit 2 / 필수 필드 누락 기대 exit 2 / dateRange 불일치 기대 exit 2 / RSS 대비 title·published 불일치 기대 exit 2를 재현했고 runner exit 0.
- 실 API 스모크 범위: 2026-07-12~2026-07-18. channels=9, videos=26, gate exit 0.
- 추가 검증: curator registry gate exit 0 (`channels=17`, YouTube 9/X 7/web 1), Python compile exit 0.

### WP-1 증거

```text
PASS[video-coverage-selftest] complete=0 missing=2 invalid-fields=2 invalid-range=2 mismatched-title=2 mismatched-published=2
SELFTEST_EXIT=0
PASS[video-coverage] channels=9 videos=26
LIVE_GATE_EXIT=0
REGISTRY_EXIT=0
PYCOMPILE_EXIT=0
```
