...
for c in ch:
    ...
    if c.get("platform") == "youtube":
        ...
        expected_rss = f"https://www.youtube.com/feeds/videos.xml?channel_id={channel_id}"
        if not isinstance(channel_id, str) or not re.fullmatch(r"UC[A-Za-z0-9_-]{22}", channel_id):
            fails.append(f"FAIL[channelId] {c.get('id','?')} invalid: {channel_id}")
        if c.get("rssUrl") != expected_rss:
            fails.append(f"FAIL[rssUrl] {c.get('id','?')} expected {expected_rss}")
