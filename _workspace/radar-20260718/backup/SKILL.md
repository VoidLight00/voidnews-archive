...
- **큐레이터 채널(상시 들름):** `references/curator-channels.json`의 YouTube 9 + X 7 채널을 매 run 무조건 훑는다(`references/collector-seeds.md`의 "큐레이터 채널" 섹션). **discovery-only** — 큐레이터에서 발견한 사건은 반드시 공식 1차 출처로 승격해 가져오고, 큐레이터 링크는 `discoveredVia: "curator-youtube"|"curator-x"`로만 보존(출처/backup 금지). 공식 1차 출처를 못 찾으면 `needs_review:true`+risk flag. collector는 16채널 방문 결과를 `01_curator_coverage.json`에 기록한다. **게이트 2종(HARD), 절대경로로 호출: ① 레지스트리 무결성 `bash ~/.claude/skills/voidnews-briefing-pipeline/references/verify_curator_channels.sh` exit 0 (핸들 변동 시) ② 수집 직후 커버리지 `bash ~/.claude/skills/voidnews-briefing-pipeline/references/check_curator_coverage.sh <run>/01_curator_coverage.json` exit 0 — 16채널 전수 방문 강제, 1개라도 미방문이면 exit 2 → conductor 폐루프가 collector 재호출.**
