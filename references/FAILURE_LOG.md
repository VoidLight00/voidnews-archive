...
### VN-RESUME-01 — 환경별 TypeScript 로더와 중간 너비 메뉴 회귀 (2026-09-09)

- 증상: Node 24 내장 TypeScript 실행 기능이 비활성인 배포판에서 영문판 검사와 IMP-0019가 실행 오류로 종료했습니다.
- 수정: 설치된 TypeScript로 주간 모듈을 변환합니다. 기존 데이터 검증과 오류 시 종료 동작은 유지합니다.
- 브라우저 발견: 실제 652px 뷰포트에서 상단 로고·Weekly·Threads·Dark 라벨이 두 줄로 깨졌습니다.
- 원인: 헤더의 640px 전환 지점과 모든 글자의 강제 줄바꿈 규칙이 충돌했습니다.
