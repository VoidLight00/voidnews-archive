...
2026년 5월 18일 Boston Dynamics가 Atlas 휴머노이드의 **전신 제어(Whole-body Control)** 시연 영상 2편과 기술 블로그를 공개했습니다. Atlas가 23kg 짜리 소형 냉장고를 무릎을 굽혀 들어 올리고, 균형을 잡으며 책상까지 옮기는 장면입니다. 시뮬레이션 학습 기반으로 최대 45kg(약 100파운드)까지 가능하다고 발표했습니다.

이 발표는 단순 데모가 아니라 **현대차그룹의 2028년 연 3만대 양산 체계**와 직결됩니다. 한국 청자에게 가장 직접적인 산업 임팩트.

**무엇이 달라졌나?**

- **전신 제어(Whole-body control)** — 다리·몸통·팔이 단일 시스템으로 협응. 무거운 물건을 들 때 다리가 균형, 몸통이 무게 분산, 팔이 그립 담당
- **Zero-shot sim-to-real transfer** — 시뮬레이션 안에서 수백만 시간 학습 후 실제 하드웨어에 추가 튜닝 없이 그대로 배포
- **Domain randomization** — 냉장고 무게, 바닥 마찰, 그립 조건을 수천 가지로 바꿔 학습 → 실제 산업 환경에 강건
- **하드웨어**: 56 자유도(DoF), 2.3m 도달, 가반 50kg, 4시간 배터리, 셀프 배터리 교체

**왜 한국 청자에게 중요한가요?**

현대차그룹이 CES 2026에서 발표하고 5월 양산형 영상으로 본격화한 로드맵:

- **2028년까지 연 3만대 생산 체계** (HMGMA 미국 공장 시작)
- **2028년 HMGMA부터 부품 분류·서열 작업에 우선 투입**
- **2030년 부품 조립까지 확대**
- **현대모비스 = 액추에이터 공급 / 현대글로비스 = 물류 자동화 / 보스턴 다이나믹스 = 본체 양산**
- **Robots-as-a-Service(RaaS)** 비즈니스 모델 — 구독·사용료 기반으로 초기 비용 부담 낮춤

기술 파트너십:

- **Google DeepMind**와 Robot Foundation Model(RFM) 공동 개발 — Gemini 기반으로 Atlas가 환경을 스스로 인지·판단
- **NVIDIA GPU** 탑재 + Omniverse 시뮬레이션
- **나스닥 상장 추진** — 5월 7일 기사 기준 다음 달 결정. 상장 자금이 양산 공장에 직접 투입 예정

**읽을 때 볼 점**

- 같은 시기 중국 휴머노이드는 이미 양산 시작, 미국 Figure AI의 BotQ는 향후 10만대 설비 보유 — **3만대는 follow-up 규모이지 leadership 아님**
- 2030년 이전에는 노무비 절감 효과 제한적이고 생산성 검증·공정 적용 확대 단계
- 영상이 보여주는 것은 "단일 작업" — 산업 현장의 다종 작업 일반화는 별도 시간 필요
- 1차 학습 데이터 + sim-to-real gap은 여전히 연구 주제 (RAI Institute, 마크 레이버트 주도)`,
