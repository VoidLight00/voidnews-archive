...
Anthropic이 2026년 5월 22일 Project Glasswing의 첫 정량 결과를 공개했습니다. Glasswing은 한 달 전 출범한 보안 협력 이니셔티브로, 핵심 기술 회사들이 Claude Mythos Preview를 방어적 보안 워크플로우에 통제된 조건으로 투입하는 프로그램입니다.

**무엇이 달라졌나?**

- 1개월 누적: 인터넷에서 가장 널리 쓰이는 소프트웨어에서 **10,000건 이상의 high/critical 취약점 발견**
- 약 50개 파트너 조직이 Claude Mythos Preview 배포
- 구체 수치:
  - **Cloudflare**: 2,000건 보고, 그중 400건이 high/critical
  - **Mozilla**: Firefox 150에서 271건 수정
  - **1,000+ 오픈소스 프로젝트 스캔**: 현재 true-positive rate 기준 약 3,900건 high/critical 예상

**왜 중요한가요?**

Claude Mythos는 일반 출시되지 않은 frontier 모델인데, Anthropic은 \"공개 위험 vs 방어적 가치\"의 균형을 Glasswing으로 시험하고 있습니다. 이번 수치는 \"AI가 공격에 쓰이기 전에 방어에 먼저 쓰일 수 있다\"는 주장의 실증 데이터입니다.

**읽을 때 볼 점**

- Anthropic 공식 표현: *\"강한 safeguards를 마련하는 즉시 Mythos-class 모델을 일반 출시로 공개할 계획이지만, 아직 그 작업은 끝나지 않았다.\"* 일반 출시 일정은 미공개.
- 5/23 TestingCatalog가 Claude 인터페이스에서 \"Mythos 1\" 식별자 발견 → UI 신호이며 공식 출시 발표 아님
- 외부 보도(The Ringer 등)는 \"인터넷을 파괴할 수 있는 AI\" 같은 자극적 프레이밍, 공식 글은 훨씬 보수적 톤`,
...
2026년 5월 25일 Thinking Machines가 첫 공식 데모와 블로그 \"Interaction Models\"를 공개했습니다. Mira Murati(전 OpenAI CTO)가 이끄는 신생 랩의 첫 외부 발표입니다.

**무엇이 달라졌나?**

- 모델 단독 출시가 아니라 **새 인터랙션 패러다임 제안**
- 핵심 아이디어: 사용자가 모델을 단발 질문이 아니라 \"형성(shape)\" — 실시간 조정, 추론 흐름 노출, 사용자 개입 지점 명시
- 블로그 + 라이브 데모 영상 동시 공개

**왜 중요한가요?**

Thinking Machines는 OpenAI 출신 톱티어 연구자들이 모인 가장 주목받는 신생 랩 중 하나입니다. 그동안 \"무엇을 만들고 있는지\"가 베일에 가려 있었는데, 이번 첫 공개는 \"이 회사가 모델을 만드는 회사가 아니라 새 인터랙션 패러다임을 만드는 회사\"라는 정체성 선언입니다. Anthropic Claude Code Agent View, Google Spark, OpenAI Codex app과 같은 흐름 — \"AI는 도구가 아니라 협업자\"로 수렴.

**읽을 때 볼 점**

- 공개된 것은 데모 + 블로그 + 비전 문서 수준. 실제 제품/API는 아직
- Thinking Machines 사이트와 공식 X에서 추가 발표 예정
- 산업 영향: 신생 랩이 OpenAI/Anthropic/Google에 대항하는 \"제4의 축\"으로 자리잡을지 시험대",Interaction Models\" 공식 데모 + 블로그를 공개했다. Mira Murati가 이끄는 OpenAI 출신 신생 랩의 첫 공개 발표로, 모델을 사용자가 실시간으로 형성·조정하는 새로운 인터랙션 패러다임을 제시했다.`,
...
2026년 5월 22일 알리바바 클라우드가 **Qwen 3.7 Max** 공식 모델 카드와 벤치마크를 공개했습니다. Qwen 시리즈의 최상위 모델로, 알리바바 자체 호스팅과 Vercel AI Gateway 등 외부 게이트웨이에서 사용 가능합니다.

**무엇이 달라졌나?**

- Qwen 3.7 Max: Qwen 라인업의 frontier급 멀티 도메인 모델
- 강점 영역: 코딩, 추론, 다국어, 긴 컨텍스트
- 같은 주 Vercel AI Gateway 등재 (개발자가 단일 키로 OpenAI·Anthropic·Qwen을 묶어 쓸 수 있음)
- 가격은 미국 frontier 모델보다 저렴한 포지셔닝

**왜 중요한가요?**

frontier AI는 OpenAI / Anthropic / Google이 독점한 시장으로 보였지만, Qwen / DeepSeek / GLM(Z.AI) 같은 중국 모델들이 벤치마크에서 거의 따라잡았습니다. \"성능 격차는 작아지고 가격 격차는 크다\"가 2026년 중반 산업 흐름입니다. 미국 게이트웨이(Vercel)가 중국 모델을 1급 옵션으로 받는다는 점도 신호.

**읽을 때 볼 점**

- 공식 벤치마크 자체는 자기 측정 — Artificial Analysis 같은 독립 평가로 교차 검증 필요
- 데이터 거버넌스·정치적 리스크 때문에 미국·EU 기업 도입에는 제약 있음
- 오픈웨이트는 별도 정책 — Qwen 3.7 Max는 API 우선, 오픈웨이트 발표 여부는 미확정`,
