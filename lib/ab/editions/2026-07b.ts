...
const tools: ABEditorPick[] = [
  ...
  {
    title: "Solgate — GPT-5.6 티어 라우팅 로컬 게이트웨이 (비공개·공개 준비 중)",
    slug: "solgate-gpt-5-6-routing-gateway",
    category: "로컬 인프라 / 비공개 프로젝트",
    deck: "Sol·Terra·Luna 연결, 긴 대화 요약, 사용량 fallback을 관리하는 고급 사용자용 관제소",
    sourceUrl: "https://github.com/VoidLight00/solgate",
    sourceLabel: "비공개 저장소 · 공개 준비 중",
    summary: "GPT-5.6 세 티어를 작업별로 연결하고 컨텍스트 압축과 fallback을 관리하는 로컬 게이트웨이입니다. 현재 일반 청중은 설치할 수 없습니다.",
    body: `Solgate는 GPT-5.6 Sol·Terra·Luna를 Claude Code에 연결하고, 작업 종류와 일시적 모델 오류에 따라 라우팅하는 로컬 게이트웨이입니다. 긴 대화는 롤링 요약으로 압축하고, 특정 모델이 cooldown 상태일 때 다른 티어를 시도합니다.\n\n**현재 공개 상태**\n저장소는 private입니다. 이 카드는 제품 방향과 실제 운용 사례를 소개하기 위한 것이며, 일반 사용자가 지금 GitHub에서 설치할 수 있다는 뜻이 아닙니다. 공개 저장소·공개 가이드·제한 배포 중 하나가 정해진 뒤 설치 대상으로 전환합니다.\n\n**주의**\n가상 100만 컨텍스트는 롤링 요약이며 무손실이 아닙니다. 모델의 실제 컨텍스트 창을 늘리지 않습니다. fallback은 쿼터 우회가 아니라 소진 순서를 관리하는 기능이며, 공유 플랜 한도가 모두 소진되면 실패합니다. 본인 구독과 본인 OAuth 범위에서만 사용해야 합니다.`,
    editorial: "GPT-5.6 카드에서 설명한 티어별 운영을 실제로 구현한 사례입니다. 다만 private 상태를 숨기지 않고 공개 준비 중인 고급 프로젝트로만 소개합니다.",
    thumbnail: { src: "/og-cache/solgate-private-editorial.svg", alt: "Solgate private local model gateway" },
    tags: ["Solgate", "GPT-5.6", "모델 라우팅", "MIT", "Private"],
    tier: "feature",
  },
];
