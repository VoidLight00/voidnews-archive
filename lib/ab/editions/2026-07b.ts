...
const tools: ABEditorPick[] = [
  ...
  {
    title: "OpenCodex — 여러 모델과 코딩 도구를 한 로컬 프록시로 연결",
    slug: "opencodex-universal-model-proxy",
    category: "오픈소스 / 모델 라우팅",
    deck: "Codex·Claude Code·여러 공급자·로컬 모델을 하나의 호환 API와 대시보드로 묶는 MIT 프록시",
    sourceUrl: "https://github.com/lidge-jun/opencodex",
    sourceLabel: "GitHub · MIT →",
    summary: "OpenCodex는 Codex CLI·앱·SDK와 Claude Code가 Anthropic, Google, OpenAI 호환, Ollama 등 여러 모델 공급자를 선택하도록 연결하는 Node.js 기반 로컬 프록시입니다.",
    body: `OpenCodex는 하나의 모델에 고정되지 않고 여러 공급자를 로컬 게이트웨이 뒤에서 바꿔 쓸 수 있게 하는 MIT 라이선스 오픈소스입니다. Node.js 18 이상에서 설치하며 npm install -g @bitkyc08/opencodex 뒤 ocx init, ocx start로 시작합니다. 기본 대시보드는 http://localhost:10100에서 열립니다.\n\n**무엇을 연결하나**\nAnthropic, Google, xAI, OpenAI 호환 API, Azure, OpenRouter, Ollama·Ollama Cloud, 로컬 vLLM·LM Studio 등 여러 공급자를 provider/model 형식으로 선택할 수 있습니다. Codex CLI·앱·SDK뿐 아니라 Anthropic 호환 API를 통한 Claude Code 연결과 GitHub Copilot App 연동도 지원합니다. 스트리밍, 도구 호출, 추론 토큰, 이미지, 모델 검색을 함께 다룹니다.\n\n**실제로 쓸 수 있는 장면**\n코딩 작업마다 Claude·Gemini·GPT·Kimi·로컬 모델을 바꿔 비교하거나, 한 공급자가 지연·쿼터·장애 상태일 때 cooldown과 failover를 적용할 수 있습니다. ChatGPT/Codex 계정 풀링과 세션 고정, 쿼터 추적도 제공하므로 여러 계정과 공급자를 운영하는 고급 사용자에게 유용합니다. 다만 모델 이름을 바꿀 수 있다는 것이 결과 품질이나 공급자 정책을 자동으로 보장한다는 뜻은 아닙니다.\n\n**반드시 확인할 것**\nOpenCodex는 OpenAI·Anthropic의 공식 제품이 아니며, 제3자 프록시 사용이 각 공급자의 약관이나 계정 상태에 영향을 줄 수 있습니다. 외부 네트워크나 LAN에 노출할 때는 OPENCODEX_API_AUTH_TOKEN 설정이 필요하고, OAuth·계정 풀링은 각 공급자의 인증과 상류 서비스 상태에 의존합니다. 먼저 loopback 환경에서 테스트하고, 비밀키는 공개 저장소나 설정 화면에 노출하지 않아야 합니다.\n\n**한 줄 판단**\n여러 코딩 모델을 실제 작업에 비교하고 로컬·클라우드·공급자 fallback을 한곳에서 관리하려는 사용자에게는 메리트가 있지만, 단일 모델만 쓰는 일반 사용자에게는 프록시 운영과 인증 관리가 추가 부담입니다.`,
