...
const vip3 = post({
  ...
  content: "**Ox Alpha의 정체가 공개됐습니다**\n\nZ.ai는 8월 26일 GLM-5.3-Flash를 공개했습니다. 공식 가중치는 총 3,200억 파라미터 가운데 토큰당 180억을 활성화하고, 텍스트·이미지·영상을 입력받는 네이티브 멀티모달 구조와 MIT 라이선스를 사용합니다. OpenRouter는 모델 FAQ에서 이전 stealth model인 Ox Alpha가 GLM-5.3-Flash였다고 명시했습니다.\n\nOpenRouter 제공 사양은 1,048,576토큰 컨텍스트와 최대 131,072토큰 출력입니다. 50% 할인 가격은 100만 토큰당 입력 0.075달러, 출력 0.25달러, 캐시 읽기 0.015달러입니다.\n\n**공식 문서 그래프 세 장을 읽는 법**\n\nZ.ai 공식 문서에는 그래프가 세 장 실려 있습니다. 첫 장은 가격 대비 성능입니다. 위로 갈수록 지능 점수가 높고 오른쪽으로 갈수록 비용이 비싼 좌표에 모델을 찍은 그림이며, GLM-5.3-Flash는 57점에 과제당 0.045달러 자리에 있습니다. 성능 1위가 아니라 값에 비해 성능이 좋다는 뜻입니다.\n\n둘째 장은 코딩·에이전트 시험 여섯 개의 성적표입니다. DeepSWE에서 Flash 63.4점 대 GLM-5.2 46.2점, AutomationBench에서 48.8점 대 26.2점으로 전작보다 올랐지만 모든 시험에서 1위는 아닙니다. 시험마다 만점과 채점 방식이 다르므로 서로 다른 시험의 점수를 나란히 비교하면 안 됩니다.\n\n셋째 장은 성적표가 아니라 엔진 설계도입니다. 필요한 내부 전문가만 호출하고, 긴 문서는 색인을 만들어 관련 부분만 읽는 구조입니다. 같은 문서를 처리할 때 KV 캐시를 약 4.44배 적게 쓰고 어텐션 계산을 약 3.01배 줄였다고 설명합니다.\n\n**검증 한계**\n\n이 세 장은 모두 Z.ai가 제시한 자료이며 VoidNews가 재측정한 값이 아닙니다. 독립 비교는 카드에 함께 걸어둔 Artificial Analysis 지수를 참고하십시오. GLM-5.2보다 전반적으로 높고 Claude Opus 4.8에 접근한다는 벤치마크 및 10분의 1 가격 주장은 Z.ai 공식 문서의 자체 평가입니다. OpenRouter의 Ox Alpha 확인은 공식 플랫폼 FAQ 근거이지만, Z.ai GitHub와 Hugging Face에는 Ox Alpha 명칭이 없습니다.\n\n**직접 확인할 것**\n\nOpenRouter의 `z-ai/glm-5.3-flash`로 실제 코딩·장기 에이전트 작업을 시험하고, 기존 Ox Alpha 사용 기록이 있다면 같은 프롬프트의 결과와 비용을 비교하십시오.",
  thumbnail: {
